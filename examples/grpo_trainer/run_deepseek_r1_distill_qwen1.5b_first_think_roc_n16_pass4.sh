#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# run_deepseek_r1_distill_qwen1.5b_first_think_roc_n16_pass4.sh
#
# GRPO-RoC + First-Think Pass@4 Reward.
#
# Rollout: 16 responses per prompt (n=16).
# RoC selection: keep 8 per prompt (target_n=8), ranked by pass_rate desc.
# Reward: pass@4 on the first-think prefix via a ref-model vllm server.
#
# GPU allocation (8 total):
#   GPU 7          → standalone vllm ref-model server (1.5 B ≈ 3 GB fp16)
#   GPUs 0–6 (×7) → GRPO training
#
# Batch math (7 training GPUs, target_n=8):
#   train_batch_size=112  →  112 × 8 = 896 samples after RoC
#   ppo_mini_batch_size=56  →  56 × 8 = 448 global mini-batch  →  64 per GPU
# ---------------------------------------------------------------------------
...
EXPERIMENT_NAME="deepseek_r1_grpo_first_think_roc_n16_pass4"
