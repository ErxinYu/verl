#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# run_deepseek_r1_distill_qwen1.5b_first_think_pass4.sh
#
...
#
# GPU allocation (9 total needed, or 8 with tight fit):
#   GPU 7          → standalone vllm server (ref model, 1.5 B ≈ 3 GB fp16)
#   GPUs 0–6 (×7) → GRPO training  (actor + rollout + ref + critic)
#
# If you only have 8 GPUs and want all 8 for training, start the vllm server
# on a different machine and set VLLM_URL below accordingly.
# ---------------------------------------------------------------------------
...
EXPERIMENT_NAME="1_5B_grpo_rollout16_0503"
