#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# run_deepseek_r1_distill_qwen7b_first_think_roc_n32_pass4_oai.sh
#
# 7B 版本，对齐 run_deepseek_r1_distill_qwen1.5b_first_think_roc_n32_pass4_oai.sh
#
# GRPO-RoC + First-Think Pass@4 Reward + overlong_as_incorrect
#   rollout.n=32 (2G), target_n=16 (G), quality_key=pass_rate
#   超长响应 score 清零作为 incorrect 参与训练（不 drop）
#
# GPU allocation (8 total):
#   GPU 7          → standalone vllm ref-model server (7B ≈ 14 GB bf16)
#   GPUs 0–6 (×7) → GRPO training
#
# Batch math (7 training GPUs, target_n=16):
#   train_batch_size=56  →  56 × 16 = 896 samples after RoC
#   ppo_mini_batch_size=28  →  28 × 16 = 448 global mini-batch  →  64 per GPU
# ---------------------------------------------------------------------------
...
EXPERIMENT_NAME="deepseek_r1_7b_grpo_first_think_roc_n32_pass4_oai"
